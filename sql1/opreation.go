...
func QueryArticle(aid int)(strcutinf.ArticleInfo,[] strcutinf.Message){
	...
	stmt1,err1:=OpenMysql().Prepare("SELECT aid,message,id,mid FROM mtable WHERE aid=?")
	defer stmt1.Close()
	if err1!=nil{
		panic(err)
	}
	stmt2,_:=OpenMysql().Prepare("SELECT mid,reply,id from rtable WHERE mid=?")
	rows,_:=stmt1.Query(aid)
	 message1:=make([]strcutinf.Message,0)
	for rows.Next(){
		var message strcutinf.Message
		err:=rows.Scan(&message.Aid,&message.Message,&message.Id,&message.Mid)
		if err!=nil{
			panic(err)
		}
		rows1,_:=stmt2.Query(message.Mid)
		for rows1.Next(){
			var reply strcutinf.ReplyInf
			err:=rows1.Scan(&reply.Mid,&reply.Reply,&reply.Id)
			if err!=nil{
				panic(err)
			}
			message.Replyal=append(message.Replyal,reply)
		}
...
//发布文章
func Luancharticle(info strcutinf.ArticleInfo)bool{
	//db,_:=sql.Open("mysql","root:@tcp(127.0.0.1:3306)/blog?charset=utf8")
	stmt,err:=OpenMysql().Prepare("INSERT INTO atable (title,context,id)VALUES (?,?,?)")
	if err!=nil{
		panic(err)
		return false
	}
	defer stmt.Close()
	defer db.Close()
	_,err=stmt.Exec(info.Title,info.Context,info.Id)
	if err!=nil{
		return false
	}
	return true
...
func Luanchmessge(message1 strcutinf.Message){
	//db,_:=sql.Open("mysql","root:@tcp(127.0.0.1:3306)/blog?charset=utf8")
	stmt,err:=OpenMysql().Prepare("INSERT INTO mtable (message,id,aid) VALUES (?,?,?)")
	defer stmt.Close()
	defer db.Close()
	if err!=nil{
		panic(err)
...
func Luanchreply(message strcutinf.ReplyInf)bool{
	stmt,err:=OpenMysql().Prepare("INSERT INTO rtable (mid,reply,id)VALUES(?,?,?)")
	if err!=nil{
		return false

	}
	defer stmt.Close()
	_,err1:=stmt.Exec(message.Mid,message.Reply,message.Id)
	if err1!=nil{
		return false
	}
	return true
